...
# 项目模块导入
from core.utils import Logger, PerformanceMonitor, FileUtils  # noqa
from core.data_processor import IndicatorCalculator  # noqa
...
class BatchProcessor:
    ...
    def process_single_stock(self, stock_code: int, stock_name: str,
        ...
        try:
            ...
            # 清理股票名称,移除非法字符
            clean_name = self.clean_filename(stock_name)

            # 保存结果
            if save_format == 'parquet':
                output_file = self.output_dir / f"stock_{stock_code}_{clean_name}.parquet"
                df_result.write_parquet(output_file, compression='snappy')
            elif save_format == 'csv':
                output_file = self.output_dir / f"stock_{stock_code}_{clean_name}.csv"
    ...
    def process_batch_parallel(self, stock_list: Optional[List[Tuple[int, str]]] = None,
        ...
        if max_workers is None:
            max_workers = min(mp.cpu_count(), 4)  # 默认最多4个线程
        ...
        # 使用线程池并行处理 (避免multiprocessing序列化问题)
        ...
        if elapsed > 0 and total_stocks > 0:
            theoretical_time = total_stocks * summary['avg_time_per_stock']
            speedup = theoretical_time / elapsed
            self.logger.info(f"并行加速比: {speedup:.2f}x")
